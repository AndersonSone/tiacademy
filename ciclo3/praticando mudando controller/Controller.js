...
///colocando os conceitos do sequelize aprendi que pode mistura com js e fazer conta e tudo
app.get('/mostrartodosclientes',async(req,res)=>{
    const mostrar = await cliente.findAll();
    res.json(mostrar)
});

///ele seleciona só os argumentod
app.get('/mostrartodosclientesselecionado',async(req,res)=>{
    const mostrar = await cliente.findAll({attributes:['id','nome']});
    res.json(mostrar)
});
///puxa so que preciso é como se fosse um if
app.get('/mostrartodosclientesfiltrado/:id',async(req,res)=>{
    const mostrar = await cliente.findAll({where:{id:req.params.id}});
    res.json(mostrar)
});
app.get('/mostrartodosclientesfiltrados',async(req,res)=>{
    const mostrar = await cliente.findAll({where:{nome:req.body.nome}});
    res.json(mostrar)
});
///mostra todos servicos ,maiores 
app.get('/mostrar2filtros',async(req,res)=>{
    const mostrar = await pedido.findAll({where:{valor:{[Op.lt]:req.body.valor}} })
    res.json(mostrar)
});
app.get('/mostrar3filtros',async(req,res)=>{
    const mostrar = await pedido.findAll({where:{valor:{[Op.gt]:req.body.valor}} })
    res.json(mostrar)
});

///consigo fazer some e tudo dentro igual Js
app.get('/mostrarentre',async(req,res)=>{
    const mostrar = await sservico.findAll({where:{id:{[Op.between]:[req.body.id,Number(req.body.id) +5]}} })
    res.json(mostrar)
});

///consigo fazer some e tudo dentro igual Js
app.get('/mostrarOR',async(req,res)=>{
    const mostrar = await sservico.findAll({where:{id:{[Op.or]:{[Op.lt]:10,[Op.eq]:16}}} })
    res.json(mostrar)
});
//usar and
app.get('/mostrarAND',async(req,res)=>{
    const mostrar = await sservico.findAll({where:{id:{[Op.and]:{[Op.lt]:10,[Op.gt]:5}}} })
    res.json(mostrar)
});
//oq eu coloca tipo nome e tudo ele ja muda
app.put('/trocaralgoservico/:id',async(req,res)=>{
    await sservico.update(req.body,{
        where:{
            id:req.params.id      
        }
    });
    res.send("servico trocado")
    
});


///limit e ofset que vai pulando  5 instancia
app.get('/mostrapulando',async(req,res)=>{
    const mostrar = await sservico.findAll({offset:2 , limit:10})
    res.json(mostrar)
});

//contar quantos id tem menos que 10
app.get('/mostraquantos',async(req,res)=>{
    const mostrar = await sservico.count({where:{id:{[Op.lt]:10}}})
    res.json(mostrar)
});

app.get('/maiorvalor',async(req,res)=>{
    const mostrar = await pedido.max('valor')
    res.json(mostrar)
});


app.get('/somadetudo/:id',async(req,res)=>{
    const mostrar = await pedido.sum('valor',{where:{servicoId:{[Op.eq]:req.params.id}}})
    res.json(mostrar)
});

app.delete('/apagartudo/:id',async(req,res)=>{
    sservico.destroy({where:{id:req.params.id}})
    
});
///gasto medio de um cliente

app.get('/media/:id',async(req,res)=>{
    const valor = await pedido.sum('valor',{where:{ClienteId:{[Op.eq]:req.params.id}}});
    const npedido = await pedido.count({where:{ClienteId:{[Op.eq]:req.params.id}}});
    media=valor/npedido;
    res.json("o cliente com id :"+req.params.id+" gastou o total de "+valor+" reais realizando "+ npedido+" pedidos com media de : "+media)
});

app.get('/media/:id',async(req,res)=>{
    const valor = await pedido.sum();
    const npedido = await pedido.count();
    media=valor/npedido;
    res.json()
});

app.get('/findone',async(req,res)=>{
    const media =await cliente.findOne({where:{id:3}})
    res.json(media)
});

app.get('/findcount',async(req,res)=>{
    const {count,raws} =await cliente.findAndCountAll({where:{cidade:"maringa"}})
    res.json(raws)
